<template>
	<view class="contacts-wrap">
		<!-- 渲染联系人 -->
		<scroll-view
			class="contacts-swiper-wrap"
			scroll-y
			:scroll-into-view="'index-' + indexCur"
			:style="[{ height: 'calc(100vh - ' + scrollViewTop + 'px)' }]"
		...
		<!-- 渲染字母索引 -->
		<view class="contacts-index-wrap flex flex-align flex-justify" :style="[{ height: 'calc(100vh - ' + scrollViewTop + 'px)' }]">
			<ul class="contacts-ul box-shadow" @touchstart="touchUlStart" @touchmove="touchUlMove" @touchend.stop="touchUlEnd">
				<li class="index-item flex flex-align flex-justify" :data-id="item" v-for="(item, index) in contactIndexData" :key="index" @touchstart="touchIdxItem">{{ item }}</li>
			</ul>
		</view>
